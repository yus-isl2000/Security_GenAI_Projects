...
llm_response = call_gemini_api("hello world", API_KEY)
print(llm_response)
...
def call_mock_llm(prompt: str, data: list):
    return call_gemini_api(f"LLM Prompt: {prompt}\nData: {data}", API_KEY)
    #return sorted(data, key=lambda _: random.random())

@register_agent("to_do")
def to_do_agent(new_input: str = None):
    prompt = "Prioritize the following tasks. Consider any new tasks provided and integrate them into the appropriate priority order."  # LLM Prompt
    if new_input:
...
def email_calendar_agent(new_input: str = None):
    prompt = "Sort the emails by importance and ensure calendar entries remain updated."  # LLM Placeholder Prompt
    if new_input:
...
def investigations_agent(new_input: str = None):
    prompt = "Build a Threat Hunting Report based on the provided initial investigation input. Use the template format:\n\n"
    prompt += "Threat Hunting Report\nScenario\nNetwork Model\nDiagram: https://online.visual-paradigm.com/\n\nAttack Details\nAttack Artefacts\n\nHunt Details\nSourceEventsAnalysis\n\nAnomaly\nMitigations, Remediations and Recommendations\nSecurity Gaps and Lesson Learned\nSecurity And Enforcement Gap Tracker\nBusiness Risk and Security Policies Infrigement\nSecurity Policies\nAutomated Hunt / Detections\n\nPlatformLog Source QueryFrequency\n\nConclusion\nAdditional Notes\nReferences\n\n[END TEMPLATE]"
    if new_input:
...
def gap_filling_agent(new_input: str = None):
    prompt = "Analyze gaps in the system and suggest actionable items for filling missing areas."  # Placeholder Prompt
    if new_input:
...
def threat_hunts_agent(new_input: str = None):
    placeholder_prompt = "Analyze new threat hunt input and update progress accordingly."  # Placeholder Prompt
    if new_input:
